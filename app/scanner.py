...
db.catalog_works (see app/catalog_import.py) holds a fourth source of work
metadata, imported in bulk from an external export for works this app has
never scanned a file for -- deliberately NOT merged into any of the
functions in this file. An earlier version folded catalog rows into the
same in-memory WorkEntry list everything else here uses, which is exactly
wrong at this table's real scale (millions of rows): every function below
runs on every single page load, and db.get_all_catalog_works's one
unbounded fetchall() over that whole table turned "load a page" into
"rebuild several GB of Python objects from scratch," which is what
actually took the app down in practice. Browsing catalog works needs its
own SQL-native, paginated path that never materializes the whole table --
not yet built.
"""
...
def rebuild_work_tags(db_path: str) -> None:
    ...
    Deliberately does NOT fold in catalog_works -- see this module's own
    docstring for why (db.get_all_catalog_works is one unbounded fetchall()
    over the whole table, fine for a one-off admin import but not for
    something re-run on every page load/startup once that table has
    millions of rows). Catalog works aren't merged into the live library
    view anywhere right now; a real solution needs its own SQL-native,
    paginated browse path instead of participating in this per-request,
    whole-library-in-memory pipeline.
    """
