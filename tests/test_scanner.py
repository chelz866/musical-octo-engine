...
def test_catalog_works_are_not_merged_into_scan_or_load_cached():
    # A real regression guard: catalog_works must never be folded into the
    # per-request WorkEntry list (scan/refresh_cache/rebuild_work_tags/
    # load_cached all run on every page load) -- db.get_all_catalog_works
    # is one unbounded fetchall() over that whole table, which is fine for
    # a one-off admin import but caused multi-GB memory spikes on every
    # page load once that table had millions of real-world rows. See
    # scanner.py's own module docstring for the incident this guards.
